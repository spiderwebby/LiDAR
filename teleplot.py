	...
	match frame.commandWord:
		...
		case 0xAD:
			...
			for i in range(sampleCnt):
				...
				#foo = f"lidar:{round((startAngle+i*0.9),2)}:{distance}|xy"	#linear plot
    
				thetaa = round((startAngle+i*0.9),2)
				x = -distance*math.cos(math.radians(thetaa))
				y = distance*math.sin(math.radians(thetaa))
				foo = f"lidar:{x}:{y}|xy"
    
				#line = foo.decode("utf-8")
				sock.sendto(foo.encode(), teleplotAddr)
